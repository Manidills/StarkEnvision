...
def trans():
    st.title("Transactions Data Fetcher")

    

    # Input fields
    type_ = st.selectbox("Select type", ["DEPLOY", "INVOKE", "DECLARE", "DEPLOY_ACCOUNT"])
    ...
    if st.button("Fetch Data"):
        if tx_hash:
            url = f"https://api.voyager.online/beta/txns/{tx_hash}" 
            headers = {
        ...
        # API Call
            url = f"https://api.voyager.online/beta/txns?to={contract}&type={type_}&ps={ps}&p={p}"
            headers = {
                "accept": "application/json",
                "x-api-key": "Kcb7B3do0Q6Vd9FHlI2UO6q92MM8hGW37NiYk2t9"
            }
